...
## September 23, 2026: JPA and Spring Data JDBC read paths

- Added read-only comparison paths: Spring Data JPA (Hibernate 7.4.5, `@Immutable` entities, one join-fetch query) at `GET /orders/{id}/jpa`, and Spring Data JDBC 4.1.1 (order aggregate owning its lines, customer and products referenced by ID) at `GET /orders/{id}/spring-data-jdbc`. The three Java paths share `OrderDetails` records.
- New integration test `orderReadPathsReturnTheSameOrderAndRecordTheirSql`: all three Java paths return the same business data as the view for order 1001, and `[]` (the view: `null`) for order 1005. It captures each request's SELECT statements from MySQL's general query log and restores the log settings afterwards. Result: view 1, plain JDBC 1, JPA 1, Spring Data JDBC 4 (order, lines, customer, products). See `experiments.txt`.
- Found while testing natively: Hibernate returned `createdAt` 4 hours early (05:30 instead of 09:30) because it read `DATETIME` through `java.sql.Timestamp` and applied the Mac's zone (America/Toronto). `hibernate.jdbc.time_zone=UTC` did not change it; `hibernate.type.java_time_use_direct_jdbc=true` did. Rechecked with the JVM forced to Asia/Dhaka: all three paths returned 09:30.
- Full native run: 20 unit and 34 integration tests passed; the three model answers were unchanged. The rebuilt Compose app served all four endpoints with no warnings.
