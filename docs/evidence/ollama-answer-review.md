...
All 55 automated checks pass. The three real-model tests check tool selection and selected facts; they do not verify every sentence, instruction or presentation detail. This review found no factual error in these three final answers, but it did find incomplete instruction following. Three fixture questions are not a general quality evaluation.
