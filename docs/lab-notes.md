...
The network change required an application change: a container's loopback address refers to itself. `OllamaRuntime` (since removed) then accepted the exact service hostname `ollama` alongside loopback, with tests retaining rejection of other origins and cloud-model metadata. Compose supplies `mysql:3306` and `ollama:11434`; the latter has no published host port. The API listens on its container interface, while its host port remains bound to loopback.
