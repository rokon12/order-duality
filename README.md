...
The model is whatever `OLLAMA_MODEL` names (default `llama3.1:8b`); the application does not check or download it. Point `OLLAMA_BASE_URL` at a local Ollama server, and start native Ollama with `OLLAMA_NO_CLOUD=1` as shown above so a cloud model cannot be used by mistake; Compose already does. Compose's explicit init service downloads the model for the container workflow.
